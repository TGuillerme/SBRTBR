...
% 1 - Branch swapping is really important
% 2 - It can be hard to implement if following classic litterature
% 3 - Here's a new spin on it for making its implementation easier.
...
In phylogenetic inference, tree rearrangement methods (or branch swapping) is used to explore tree space \citep[i.e. the realm of all the possible trees][]{page1993islands,morrison2007increasing,Sanderson448}  in order to find the most parsimonious or most likely topology regarding the data provided.
The need of exploring the tree space is because of the astonishingly high amount of topologies that can be obtained with any tree with $n$ number of taxa ($(2n-3)!!$ rooted trees and ($(2n-5)!!$ unrooted ones).
...
%We therefore propose some vision of how to implement them
...
%TG: + Something about writing code for humans not computers? (i.e. non-cryptic/obfuscating coding practices)
