...
\begin{figure}[!htbp]
\centering
   \includegraphics[width=0.7\textwidth]{Figure/Figure1.pdf}
\caption{Example of redundant swaps with Suptree Pruning and Regrafting (SPR). In this example, two independent pruning (1, clade ``(G,F)'' and 2, clade ``(E,D)'') and regrafting (orange doted line) leads to the exact same topology. Note that both regraftings are done on a neighbouring branch (the orange dots denotes the original position of the clades).}
\label{Figure_SPR}
\end{figure}
